<?php
$host = 'http://localhost/mine/blog';
$path = '/opt/lampp/htdocs/mine/blog';
class database{
	function connect(){
		$con = new mysqli('localhost','bloguser','blog1234','blog');
		return $con;
	}
...
class login{
    function validation(){
        $this->loginprocess();
        $this->logoutprocess();
        if(empty(filter_input(INPUT_COOKIE,'login',FILTER_DEFAULT))){
            return false;
        }else{
            $q = new database();
            $con = $q->connect();
            $rtotal = $q->query("SELECT id FROM login WHERE username='%s' AND ip='%s'",$con,array(filter_input(INPUT_COOKIE,'login',FILTER_DEFAULT),filter_input(INPUT_SERVER,'REMOTE_ADDR',FILTER_DEFAULT)));
    ...
    function loginprocess(){
        if(!empty(filter_input(INPUT_POST,'username',FILTER_DEFAULT))){
            $q = new database();
            $con = $q->connect();
            $rtotal = $q->query("SELECT username FROM user WHERE username='%s' AND password=MD5('%s')",$con,array(filter_input(INPUT_POST,'username',FILTER_DEFAULT),filter_input(INPUT_POST,'password',FILTER_DEFAULT)));
            if($rtotal->num_rows!=0){
                $q->query("DELETE FROM login WHERE username='%s'",$con,array(filter_input(INPUT_POST,'username',FILTER_DEFAULT)));
                $q->query("INSERT INTO login VALUES(null,'%s','%s',null)",$con,array(filter_input(INPUT_POST,'username',FILTER_DEFAULT),filter_input(INPUT_SERVER,'REMOTE_ADDR',FILTER_DEFAULT)));
                setcookie('login',filter_input(INPUT_POST,'username',FILTER_DEFAULT));
                $con->close();
    ...
    function logoutprocess(){
        if(!empty(filter_input(INPUT_GET,'logout',FILTER_DEFAULT))){
            $q = new database();
            $con = $q->connect();
            $q->query("DELETE FROM login WHERE username='%s'",$con,array(filter_input(INPUT_COOKIE,'login',FILTER_DEFAULT)));
            setcookie('login','');
            $con->close();
            header('location:index.php');
        }
